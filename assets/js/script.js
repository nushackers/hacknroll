// Draw Worm
// modified from https://codepen.io/tholman/pen/EpfLs
//
...
var dcolor = {
    ...
    nextRed: Math.floor(Math.random() * 255),
    nextGreen: Math.floor(Math.random() * 255),
    nextBlue: Math.floor(Math.random() * 255),
    colorChange: 20,
    getColor: function(){
        return 'rgba('+Math.floor(this.red)+','+Math.floor(this.green)+','+Math.floor(this.blue)+', 0.5)';
    },
    updateColor: function(){
        if(this.colorChange === 0){
            this.nextRed = Math.floor(Math.random() * 255);
            this.nextGreen = Math.floor(Math.random() * 255);
            this.nextBlue = Math.floor(Math.random() * 255);
            this.nextAlpha = Math.random();
            this.colorChange = 20;
        }else{
            this.red += (this.nextRed - this.red) / this.colorChange;
            this.green += (this.nextGreen - this.green) / this.colorChange;
            this.blue += (this.nextBlue - this.blue) / this.colorChange;
            this.alpha += (this.nextAlpha - this.alpha) / this.colorChange;
            this.colorChange--;
        }
    }
};
...
function DrawWorm(){
  ...
  this.mouse = mouse;
    ...
    this.initialize = function(dom){
        ...
        this.initSize(dom.clientWidth, dom.clientHeight);
    ...
    var Check = function(){
        ...
        context.beginPath(); 
        context.strokeStyle = dcolor.getColor();
...
setTimeout( function() {
  app = new DrawWorm();
  app.initialize(document.querySelector('.hero'));
  count = 0;
  interval = setInterval( demoApp, 20 );

  window.onresize = function() {
    var dom = document.querySelector('.hero');
    app.initSize(dom.clientWidth, dom.clientHeight);
  }
}, 10);